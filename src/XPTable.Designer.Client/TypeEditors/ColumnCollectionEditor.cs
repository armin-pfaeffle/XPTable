...
public class ColumnCollectionEditor : Microsoft.DotNet.DesignTools.Client.Editors.CollectionEditor
{
	...
	protected override string? HelpTopic
		=> ( _helpTopic.Length == 0 ) ? base.HelpTopic : _helpTopic;
	...
	protected override void ShowHelp( )
	{
		...
		PropertyGrid? grid = Helpers.Helpers.GetPropertyGrid( _form?.Controls );

		// Check currently selected grid item
		if ( grid is not null )
		{
			GridItem item = grid.SelectedGridItem;
			if ( item is not null && ( item.GridItemType == GridItemType.Property ||
									   item.GridItemType == GridItemType.ArrayValue ) )
			{
#warning designer question
				...
				if ( item.PropertyDescriptor.GetPropValue( "PropertyData" )?.GetPropValue( "ComponentType" ) is
					Microsoft.DotNet.DesignTools.Protocol.Types.TypeIdentity typeIdentity )
